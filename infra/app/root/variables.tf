...
# Los valores reales estan en environments/qa.tfvars y environments/prod.tfvars
# ==============================================================================
...
variable "vpc_cidr" {
  ...
  validation {
    ...
    error_message = "vpc_cidr debe ser un CIDR valido (ej: 10.10.0.0/16)"
  }
}
...
variable "public_subnet_cidrs" {
  ...
  validation {
    ...
    error_message = "Debe especificar al menos 2 subnets publicas"
  }
}
...
variable "backend_instance_count" {
  description = "Numero de instancias EC2 para el backend"
  type        = number
  default     = 1
  validation {
...
variable "allowed_ssh_cidrs" {
  description = "Lista de CIDRs permitidos para SSH al Bastion (tu IP publica)"
  type        = list(string)
  default     = []
}

variable "allow_ssh_anywhere" {
  description = "Si es true, ademas de las IPs definidas y la IP detectada se habilita 0.0.0.0/0 para SSH (probar rapido). Ponlo en false para cerrar."
  type        = bool
  default     = true
}

variable "include_detected_ip" {
  description = "Si es true, agrega automaticamente tu IP publica /32 a las reglas de SSH."
  type        = bool
  default     = true
}

variable "generate_ssh_key" {
  description = "Si es true, Terraform generara y registrara automaticamente el key pair en AWS"
  type        = bool
  default     = true
}
...
variable "db_allocated_storage" {
  ...
  validation {
    ...
    error_message = "RDS MySQL requiere minimo 20 GB"
  }
}
