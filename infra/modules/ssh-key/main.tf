# ------------------------------------------------------------------------------
# SSH KEY MODULE - Generacion y registro del key pair de EC2
# ------------------------------------------------------------------------------
# 1. Genera un par de llaves RSA (tls_private_key)
# 2. Publica la llave publica en AWS (aws_key_pair)
...
# Cuando el modulo se usa unicamente para asociar una llave existente
# (create = false) exponemos igualmente una dependencia ligera para evitar
# problemas de orden de creacion cuando se utilicen los outputs.
