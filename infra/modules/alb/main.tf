...
# ==============================================================================
# Load balancer L7 (HTTP) que distribuye trafico entre frontend y backend.
...
resource "aws_security_group" "alb" {
  ...
  # Acepta trafico HTTP desde cualquier IP
  ingress {
...
resource "aws_lb" "this" {
  ...
  idle_timeout       = 60  // Segundos antes de cerrar conexion inactiva
...
# ------------------------------------------------------------------------------
# Agrupa instancias frontend para health checks y distribucion de carga.
# ------------------------------------------------------------------------------
resource "aws_lb_target_group" "frontend" {
  ...
  health_check {
    ...
    matcher             = "200-399"  // Codigos HTTP considerados saludables
...
resource "aws_lb_target_group" "backend" {
  count = var.enable_backend_listener ? 1 : 0
...
# ------------------------------------------------------------------------------
# Por defecto, todo el trafico va al frontend.
# ------------------------------------------------------------------------------
...
resource "aws_lb_listener_rule" "api_to_backend" {
  ...
  priority     = 10  // Menor numero = mayor prioridad
